...
## Pending Tasks (updated)
- [ ] Review & merge PR #123 (fix/issue-115-builder-indicators-fallback). (evidence: https://github.com/poong92/pruviq/pull/123)
- [ ] Review & merge PR #127 (autofix/resolve-static-merge-20260228) — resolves merge conflicts for public/data static files (created by autonomous run). (evidence: https://github.com/poong92/pruviq/pull/127)
- [ ] Ops: provision BRAVE_API_KEY (issue #21) — steps added & labelled P1-high; needed for the research PoC to run (see https://github.com/poong92/pruviq/issues/21).
- [ ] Backend/Ops: normalize `macro.derivatives` response (issue #113) — escalated to P0-critical with diagnostic comment (see https://github.com/poong92/pruviq/issues/113).
- [ ] Implement static-data pipeline change (issue #128) — move generated static data out of main or write to `public/data/cache` (P1-high). (see https://github.com/poong92/pruviq/issues/128)
- [ ] Monitor PR #108 (fix/issue-21-enable-brave-api-docs) and re-run CI after secret provisioning.

Notes: I created PR #127 to resolve the immediate merge conflict in static data and added issues/labels for ops/backend where I could not act (BRAVE_API_KEY, macro.derivatives). Continue monitoring PRs #123, #108, and #127.
