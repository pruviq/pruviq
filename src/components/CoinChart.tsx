      ...
      ({ createChart, CandlestickSeries, LineSeries, HistogramSeries }) => {
        ...
        const candleSeries = chart.addSeries(CandlestickSeries, {
          ...
          priceFormat: {
            type: "price",
            minMove:
              ohlcv[0]?.c < 0.01 ? 0.0000001 : ohlcv[0]?.c < 1 ? 0.00001 : 0.01,
          },
        });
        ...
        const ema20 = chart.addSeries(LineSeries, {
          color: getCssVar("--color-chart-ema20"),
          lineWidth: 1,
          priceLineVisible: false,
          lastValueVisible: false,
          visible: false,
        ...
        const ema50 = chart.addSeries(LineSeries, {
          color: getCssVar("--color-chart-ema50"),
          lineWidth: 1,
          priceLineVisible: false,
          lastValueVisible: false,
          visible: false,
        ...
        volumeSeries.setData(
          ohlcv.map((b) => ({
            ...
            color:
              b.c >= b.o
                ? getCssVar("--color-chart-vol-up")
                : getCssVar("--color-chart-vol-down"),
          })),
        );
      ...
      <div class="py-8 text-center">
        ...
        <button
          onClick={() => {
            setError(null);
            setLoading(true);
            fetch(`${API_URL}/ohlcv/${SYMBOL}?limit=3000`)
              .then((res) => {
                if (!res.ok) throw new Error("Failed");
                return res.json();
              })
    ...
    <div class="fade-in">
      ...
      <div class="bg-[--color-bg] border border-[--color-border] rounded-xl overflow-hidden mb-3 relative">
        ...
        <div class="absolute top-2 right-3 z-10 flex flex-wrap gap-1.5">
          <ToggleBtn
            active={showBB}
            activeColor={getCssVar("--color-accent")}
            label={t.bbBands}
            onClick={() => setShowBB(!showBB)}
          />
          <ToggleBtn
            active={showEMA}
            activeColor={getCssVar("--color-chart-ema20")}
            label={t.ema}
