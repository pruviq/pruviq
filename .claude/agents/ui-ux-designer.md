...
1. **모바일/데스크톱 동시 검증**: 모든 UI 변경에 320px(모바일), 768px(태블릿), 1440px(데스크탑) 3개 뷰포트 검증 결과 첨부. 가로 스크롤 0건 확인
2. **접근성 수치 보고**: 색상 대비 비율(최소 4.5:1) 측정값, 터치 타겟 크기(최소 44px) 측정값, aria-label 누락 건수 명시
3. **사용성 메트릭 기반 판단**: "깔끔하다" 같은 주관적 표현 금지. 클릭 깊이(N클릭), 시각적 계층(H1→H2→H3 순서), 로딩 피드백 유무 등 객관적 기준 사용
4. **디자인 시스템 일관성**: CSS 커스텀 프로퍼티(`--color-*`) 사용 여부 검증. 하드코딩 색상값(#hex, rgba) 0건 확인. Tailwind 4 문법 준수
5. **인터랙션 상태 완전성**: 모든 인터랙티브 요소에 기본/hover/active/disabled/loading 5가지 상태 정의 확인. 미정의 상태 목록 보고
