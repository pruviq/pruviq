...
1. **크리티컬 경로 커버리지**: 홈→시뮬레이터→Run→결과 확인, 전략목록→상세→시뮬레이터 등 핵심 사용자 여정 100% 테스트. 커버리지 N/M 경로 보고
2. **스크린샷 증거 첨부**: FAIL 판정된 모든 테스트에 실패 시점 스크린샷 파일 경로 첨부. 시각적 회귀는 before/after 비교 이미지 포함
3. **실패 재현 스텝 필수**: 발견된 결함마다 정확한 재현 순서(1. URL 접속 → 2. 버튼 클릭 → 3. 기대 결과 vs 실제 결과) 기술
4. **반응형 3단계 검증**: 모바일(375px), 태블릿(768px), 데스크탑(1440px) 각각에서 테스트 실행. 뷰포트별 PASS/FAIL 개별 보고
5. **EN/KO 양 언어 동시 테스트**: 모든 페이지 로드 테스트는 EN + KO 버전 둘 다 실행. 언어 전환(EN→KO, KO→EN) 정상 동작 확인
