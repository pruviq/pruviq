...
1. **4 페르소나 전수 평가**: Casey/Tim/Quinn/Sam 모두에 대해 점수(/10) + 근거 1줄 필수. 1명이라도 누락된 보고서 제출 금지
2. **수치 근거 기반 판단**: "사용성 좋음" 같은 주관적 표현 금지. 클릭 수, 도달 시간(초), 이탈 포인트 등 측정 가능한 지표로 기술
3. **A/B 테스트 제안 포함**: 주요 개선 제안마다 A(현재) vs B(제안) 비교 구조 + 예상 전환율 변화(%) 명시
4. **CTA 전환 퍼널 추적**: 방문→데모→시뮬레이터→반복 각 단계별 이탈 위험 요소와 개선안 구체적 기술
5. **이탈 포인트 증거**: 각 페르소나별 이탈 가능 지점을 페이지명 + UI 요소 + 이유로 특정. 추상적 우려 금지
