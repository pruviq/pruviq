...
### 이미 구축된 것
- [x] 웹사이트 v1.1 (1,209 페이지, EN/KO 이중 언어)
- [x] **Strategy Builder** - 노코드 전략 설계 + 백테스트 (2026-02-18 출시)
- [x] **ConditionEngine** - 11개 지표, JSON 조건 파싱, 커스텀 백테스트 API
- [x] 5개 전략 시뮬레이션 API (BB Squeeze, Momentum, ATR, HV)
- [x] Strategy Comparison (5개 전략 실시간 비교, /backtest API 활용)
- [x] Coin Explorer (535+ 코인)
- [x] Strategy Library (5개 전략 상세 페이지)
- [x] Performance Dashboard (38일 실거래 데이터)
- [x] 수수료 비교 페이지 (Binance 레퍼럴 라이브)
- [x] 613개 코인 1H OHLCV 데이터 (2년+, Mac Mini rsync 완료)
...
#### 데이터 준비
- [x] autotrader 613개 코인 데이터 → Mac Mini rsync (2026-02-18 완료)
...
- `backend/src/engine/indicator_pipeline.py` - 동적 인디케이터 계산 (11개) ✅
...
구현 파일 (✅ 2026-02-18 완료):
- `src/components/StrategyBuilder.tsx` - 메인 빌더 UI (Preact island, ~770줄) ✅
- `src/components/StrategyComparison.tsx` - 5개 전략 비교 (실시간 API) ✅
- `src/pages/builder.astro` / `src/pages/ko/builder.astro` - 페이지 ✅
- `src/pages/strategies/compare.astro` / `ko/` - 비교 페이지 ✅

미구현 (Phase 2로 연기):
- `src/components/EquityCurve.tsx` - 에퀴티 차트 (lightweight-charts)
- CSV 다운로드 기능
...
```
2026-02
├─ W3 (2/18): Phase 1 완료 ✅ (1일 만에 달성!)
│   ├─ 데이터 rsync (613 코인) ✅
│   ├─ ConditionEngine + IndicatorPipeline (11개 지표) ✅
│   ├─ /backtest API + /builder/presets + /builder/indicators ✅
│   ├─ Strategy Builder UI (770줄 Preact island) ✅
│   ├─ Strategy Comparison (5개 전략 실시간 비교) ✅
│   ├─ i18n 완전화 (EN/KO 모든 페이지) ✅
│   ├─ Home page Builder CTA 추가 ✅
│   └─ 535+ 코인 수 통일 ✅
│
├─ W3-4 (2/18~): Phase 1B 인프라
│   ├─ n8n 기본 파이프라인
│   ├─ 데이터 자동 동기화 cron
│   └─ OpenClaw 재설치
