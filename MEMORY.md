...
Last updated: 2026-02-28 17:17 KST
...
- cron:gh-issues-autofix
  ...
  - What I did:
    ...
    7. Next steps / Blockers:
       - Monitor PR #123 (fix/issue-115-builder-indicators-fallback) for CI & review results: https://github.com/poong92/pruviq/pull/123 (confirmed open).
       - Ops/Backend: investigate `macro.derivatives` null response and normalize API output or ensure static pipeline includes derivatives.
...
- cron:63c0583a-741f-4b76-9f53-861ab7456f81 (daily-seo-audit)
  ...
  - Evidence (commands / outputs):
    ...
    - `echo -n "$GH_TOKEN" | wc -c` → `0` (no GH_TOKEN in environment)
    - `gh auth status` → logged in as `poong92` (token scopes include `repo` and `workflow`) (confirmed via `gh auth status`).
...
- cron:gh-issues-autofix
  - Time: 2026-02-28 16:17 KST
  - Action requested: autonomous gh-issues-autofix cron run (fix failed PRs and triage open issues)
  - What I did:
    1. Read SOUL.md and MEMORY.md to confirm context and safety rules (local file reads confirmed).
    2. Synchronized `main` (git checkout main && git pull origin main) — local repo up-to-date (confirmed via git output).
    3. Inspected open PRs: `gh pr list --state open --json number,title,headRefName,statusCheckRollup,comments` — no PRs with failing status checks were found that required code fixes during this run (confirmed).
    4. Inspected open issues: `gh issue list --state open --limit 50 --json number,title,body,labels` and prioritized by label (P0 > P1 > P2).
    5. Fix implemented (Issue #115):
       - Diagnosis: `GET https://api.pruviq.com/indicators` returns 404 (confirmed via `curl -s -o /dev/null -w "%{http_code}" https://api.pruviq.com/indicators` → `404`).
       - Root cause: some clients requested a non-existent `/indicators` path; frontend previously made direct API fetches for builder endpoints which can fail when API is unavailable.
       - Fix implemented: updated simulator to use static-first fallback (fetchWithFallback + STATIC_DATA) for builder indicators and presets, and added a secondary coins fallback to STATIC_DATA.coinsStats.
       - Files changed: src/components/SimulatorPage.tsx (use fetchWithFallback('/builder/indicators', STATIC_DATA.builderIndicators) and fetchWithFallback('/builder/presets', STATIC_DATA.builderPresets)). Confirmed in commit 3705c07.
       - Branch: fix/issue-115-builder-indicators-fallback (created locally and pushed to origin).
       - PR created: https://github.com/poong92/pruviq/pull/123 (fix/issue-115-builder-indicators-fallback).
       - Local build: `npm run build` completed successfully (build output: `[build] 2446 page(s) built in 35.87s` and `[build] Complete!`) (confirmed via build logs).
    6. Issue triage (Issue #113):
       - Diagnosis: `GET https://api.pruviq.com/macro` returns `"derivatives": null` (confirmed via `curl https://api.pruviq.com/macro | jq '.derivatives'` → `null`).
       - Root cause: server-side data shape (backend returning null for derivatives). Frontend already uses `fetchWithFallback('/macro', STATIC_DATA.macro)`, so the site is resilient; however external API consumers may break.
       - Action: posted a diagnostic comment on issue #113 explaining the backend should normalize the response to return an array or omit the field, and recommended tests. No code change required in frontend at this time.
    7. Comments posted:
       - Issue #115: comment + PR link (https://github.com/poong92/pruviq/issues/115#issuecomment-3976585002).
       - Issue #113: diagnostic comment posted (https://github.com/poong92/pruviq/issues/113#issuecomment-3976585164).

  - Result:
    - Implemented and pushed a frontend fix for issue #115; PR #123 opened for review.
    - Added diagnostic triage comment to issue #113 (backend fix required).
    - No failing PRs required fixes in this run.

  - Next steps / Blockers:
    - Monitor PR #123 for CI results and merge when approved: https://github.com/poong92/pruviq/pull/123.
    - Backend owners: consider implementing a stable `/indicators` or redirecting clients to `/builder/indicators` if external clients expect that path. Also normalize `macro.derivatives` to a non-null value if API consumers require it.

  - Evidence (commands / outputs):
    - `curl -s -o /dev/null -w "%{http_code}" https://api.pruviq.com/indicators` → `404` (confirmed)
    - `curl https://api.pruviq.com/macro | jq '.derivatives'` → `null` (confirmed)
    - Commit: 3705c07 (fix applied to src/components/SimulatorPage.tsx)
    - PR: https://github.com/poong92/pruviq/pull/123
    - `npm run build` → `[build] 2446 page(s) built in 35.87s` and `[build] Complete!` (from build log)

  - Generated by PRUVIQ Bot (프루빅) on 2026-02-28 16:17 KST.
...
- cron:gh-issues-autofix
  ...
  - What I did:
    ...
    6. Actions taken:
       - No source-code changes were required or applied.
  ...
  - Blockers / Next steps:
    ...
    - After the secret is added, re-run CI or push a no-op commit to re-run workflows; I will pick this up on the next cron run.
...
- cron:gh-issues-autofix
  ...
  - Blockers / Next steps:
    - Ops/SRE or repo admin must provision BRAVE_API_KEY (repo Actions secret or Gateway env) so PR #108 and the research PoC can fully function.
    - After the secret is added, re-run CI or push a no-op commit to re-run workflows; I will attempt fixes if CI fails.
...
- [ ] Monitor PR #123 (fix/issue-115-builder-indicators-fallback) and merge after review. 


---

- cron:gh-issues-autofix
  - Time: 2026-02-28 22:23 KST
  - Actions taken:
    1. Synchronized main and resolved merge conflicts caused by generated static data under public/data/*.json by preferring origin/main (commit 0dab7cc) — conflict files: public/data/coins-stats.json, public/data/macro.json, public/data/market.json, public/data/news.json (confirmed via git status/commit). 
    2. Stashed local working changes before creating fix branches (stash: stash@{0} — "wip: stash before gh-issues-autofix"). (confirmed via `git stash list`) 
    3. Implemented a low-risk automation to publish generated static data into a dedicated branch instead of directly updating main: added `.github/workflows/static-refresh-to-branch.yml` and `docs/generated-data.md` on branch `fix/issue-128-move-generated-data`. Committed and pushed the branch and opened PR #129: https://github.com/poong92/pruviq/pull/129 (confirmed via `gh pr view 129`).
    4. Issue #21 (BRAVE_API_KEY) is blocked by missing secrets; I posted a diagnostic comment explaining required ops steps and next tests (confirmed via `gh issue view 21 --json comments`).
    5. Verified local build passed: `npm run build` completed successfully (from build output: "[build] 2446 page(s) built in 26.72s; [build] Complete!")

  - Result:
    - PR #129 opened to implement a safe automation path for generated static data (see PR URL above).
    - Issue #21 remains blocked pending secret provisioning (ops action required).

  - Evidence:
    - Commit: 0dab7cc (merge conflict resolution) (confirmed via `git log -1 --pretty=format:"%h %s"`)
    - Branch: fix/issue-128-move-generated-data (pushed) (confirmed via `git branch --show-current` after checkout)
    - PR: https://github.com/poong92/pruviq/pull/129 (confirmed via `gh pr view 129`)
    - Issue comment on #21: posted (confirmed via `gh issue view 21 --json comments`)
    - Build: `npm run build` output includes "[build] Complete!" (build logs present)
